...
| **评论功能** | 日记详情页**发表评论（无需审核，即时显示）**；删除权限：评论者本人 / 日记作者 / 管理员；评论内容经 HTML 转义防 XSS |
...
| 普通用户前台 | 浏览公开日记、发布 / 管理自己的日记、管理个人资料与收藏、发表评论 |
| 管理员后台 | 独立后台（`/admin`）；数据统计（含互动 / 评论 / 禁言数）；**仪表盘修改密码**；用户管理（**设为 / 取消管理员**、**处罚：禁言 1 天 / 1 周 / 永久**、禁用、删除）；日记审核（通过 / 驳回 / 删除） |
...
```
dairy/
├── package.json            # 依赖、版本号（1.1.0）与启动脚本
...
│   ├── public.js           # 他人主页（/api/users）
│   ├── diary.js            # 日记 + 点赞/转发/收藏 + 评论（/api/diaries）
...
│   ├── user.html           # 他人主页（点击头像/昵称进入）
│   ├── diary.html          # 日记详情（含地点小地图 + 评论区）
│   ├── vendor/leaflet/     # Leaflet 1.9.4 本地托管（无需外网 CDN）
│   ├── css/style.css
│   └── js/                 # 页面脚本 + common.js + countries.js + user.js
...
| GET | /api/users/:id | **他人主页**（公开资料 + 公开日记） | 否 |
...
| POST | /api/diaries/:id/comments | **发表评论**（无需审核） | 登录 |
| DELETE | /api/comments/:id | **删除评论**（本人/日记作者/管理员） | 登录 |
