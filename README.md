...
| **消息中心** | 导航栏铃铛入口 + **未读红点**（每分钟刷新）；**作品被点赞 / 被评论 / 评论被回复 / 全站广播 / 私信** 均推送消息；支持全部已读 |
| **私信功能** | 他人主页「私信」按钮进入对话；双方消息按会话线程组织，进入对话自动标记已读；被拉黑 / 禁言期间不可发送 |
...
| **内容审核（无审核员方案）** | **敏感词过滤**已落地：发布 / 编辑日记、评论、私信、广播命中即拦截（含全角、大小写、分隔符变体对抗）；图片三重格式校验；完整方案见 `docs/无审核员内容审核方案.pdf` |
...
```
dairy/
├── package.json            # 依赖、版本号（1.2.0）与启动脚本
