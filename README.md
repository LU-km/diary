...
| 日记功能 | 文字 + 多图上传；**地图标点选择发布地点（Leaflet 本地托管 + OpenStreetMap 可商用开源地图，选点后可手动填写地点名称）**；**发布时自动记录发布时间**；可见性可选公开 / 仅自己可见；编辑、删除、关键词搜索、分页 |
...
```
dairy/
├── package.json            # 依赖、版本号（1.0.03）与启动脚本
...
│   ├── profile.html        # 个人资料中心（IP/居住地/改密/收藏/注销）
...
- 瓦片采用 **OpenStreetMap** 标准瓦片（可商用开源，使用需保留署名），**查看/选点地图仍需要联网**加载瓦片；
- 逆地理编码使用 OpenStreetMap 的 Nominatim 服务（3 秒超时），**识别失败时自动回退为坐标，且可在地点名称输入框中手动填写地名**（如「杭州西湖」）；请遵守 Nominatim 使用频率限制（约 1 次/秒）。
